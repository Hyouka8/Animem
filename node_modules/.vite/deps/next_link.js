	...
	*/ var WEBPACK_LAYERS_NAMES = {
	...
	var WEBPACK_LAYERS = {
		...
		GROUP: {
			...
			nonClientServerTarget: [WEBPACK_LAYERS_NAMES.middleware, WEBPACK_LAYERS_NAMES.api],
			app: [
				WEBPACK_LAYERS_NAMES.reactServerComponents,
				WEBPACK_LAYERS_NAMES.actionBrowser,
				WEBPACK_LAYERS_NAMES.appMetadataRoute,
		...
		*/ const appPrefetchKind = prefetchProp === null ? _routerreducertypes.PrefetchKind.AUTO : _routerreducertypes.PrefetchKind.FULL;
		{
			function createPropError(args) {
				return /* @__PURE__ */ new Error("Failed prop type: The prop `" + args.key + "` expects a " + args.expected + " in `<Link>`, but got `" + args.actual + "` instead." + (typeof window !== "undefined" ? "\nOpen your browser's console to view the Component stack trace." : ""));
			}
		...
		if (isAppRouter && !asProp) {
			let href;
			if (typeof hrefProp === "string") href = hrefProp;
			else if (typeof hrefProp === "object" && typeof hrefProp.pathname === "string") href = hrefProp.pathname;
			if (href) {
				if (href.split("/").some((segment) => segment.startsWith("[") && segment.endsWith("]"))) throw new Error("Dynamic href `" + href + "` found in <Link> while using the `/app` router, this is not supported. Read more: https://nextjs.org/docs/messages/app-dir-dynamic-href");
			}
		}
		...
		if (legacyBehavior) {
			...
			try {
				...
				if (!children) throw new Error("No children were passed to <Link> with `href` of `" + hrefProp + "` but one child is required https://nextjs.org/docs/messages/link-no-children");
				throw new Error("Multiple children were passed to <Link> with `href` of `" + hrefProp + "` but only one child is supported https://nextjs.org/docs/messages/link-multiple-children" + (typeof window !== "undefined" ? " \nOpen your browser's console to view the Component stack trace." : ""));
			}
		} else if ((children == null ? void 0 : children.type) === "a") throw new Error("Invalid <Link> with <a> child. Please remove <a> or use <Link legacyBehavior>.\nLearn more: https://nextjs.org/docs/messages/invalid-new-link-with-extra-anchor");
		...
		const childProps = {
			ref: setRef,
			onClick(e) {
				if (!e) throw new Error("Component rendered inside next/link has to pass click event to \"onClick\" prop.");
