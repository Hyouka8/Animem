...
async function ensureCollection(definition) {
  try {
    ...
    console.log(`Collection "${definition.name}" existiert bereits — übersprungen.`);
  } catch {
...
async function extendUsersCollection() {
  ...
  await pb.collections.update("users", {
    ...
    // Registrierung soll öffentlich möglich sein; jeder darf sein eigenes
    // Profil sehen/bearbeiten. Rollenvergabe läuft ausschließlich über
    // unsere eigene API-Route mit Rechteprüfung (lib/permissions.ts), nicht
    // direkt über PocketBase-Regeln.
    createRule: "",
